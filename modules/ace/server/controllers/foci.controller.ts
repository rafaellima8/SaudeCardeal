...
// Schema de validação bidirecional (camelCase E snake_case)
const camelCaseSchema = z.object({
  visitId: z.string().uuid("Visit ID deve ser UUID válido"),
...
const fociCreateSchema = z.union([camelCaseSchema, snakeCaseSchema]);

// Helper function to convert to snake_case for service layer
function toSnakeCase(data: any): any {
  // Coerce quantity to number if provided, preserve undefined as undefined
  ...
  return {
    visit_id: data.visitId ?? data.visit_id,
    dwelling_id: data.dwellingId ?? data.dwelling_id,
    foci_type: data.fociType ?? data.foci_type,
    location_description: data.locationDescription ?? data.location_description,
    latitude: data.latitude,
    longitude: data.longitude,
    quantity,
    action_taken: data.actionTaken ?? data.action_taken,
    status: data.status,
    notes: data.notes,
  };
}
...
export class FociController {
  ...
  async createFocus(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Convert to snake_case for service layer
      const snakeCaseData = toSnakeCase(validatedData);
      const focus = await fociService.createFocus(snakeCaseData, userId);
